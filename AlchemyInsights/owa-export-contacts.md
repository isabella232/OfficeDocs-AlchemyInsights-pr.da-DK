---
title: '8000020: Sådan eksporterer du kontakter'
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "8000020"
- "2003"
ms.assetid: ''
ms.openlocfilehash: d7dfdf30ec51f393cb47cb3fe89577923d4ac054579391f1fa9700bb9c3c6319
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: da-DK
ms.lasthandoff: 08/05/2021
ms.locfileid: "53983267"
---
# <a name="export-contacts"></a>Eksportér kontakter

1. Nederst på siden i Outlook på internettet du vælge fanen <img src='data:image/png;base64,' />
 **Personer**.
2. På værktøjslinjen skal du vælge **Administrer**  >  **Eksportér kontakter**. 
3. Vælg at eksportere alle kontakter eller kun kontakter fra en bestemt mappe, og vælg derefter **Eksportér.**
4. Nederst på siden skal du vælge Gem **for at** gemme "contacts.csv" i standardmappen for downloads. Eller vælg pilen ud for **Gem,** og vælg **Gem som**.

Du kan få mere at vide [under Eksportér kontakter fra Outlook](https://support.office.com/article/10f09abd-643c-4495-bb80-543714eca73f#ID0EAACAAA=Outlook_on_the_web).

