---
title: '8000019: Sådan tilføjer eller importerer du kontakter'
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "8000019"
- "2002"
ms.assetid: ''
ms.openlocfilehash: 7eb53224bb3370fc434e520d69910d0c9c23f501
ms.sourcegitcommit: c6692ce0fa1358ec3529e59ca0ecdfdea4cdc759
ms.translationtype: MT
ms.contentlocale: da-DK
ms.lasthandoff: 09/14/2020
ms.locfileid: "47694053"
---
# <a name="import-contacts"></a>Importere kontakter

Før du importerer kontakter til Outlook på internettet, skal du eksportere dem fra din anden mail udbyder som en. csv-fil. Derefter skal du følge vejledningen nedenfor.

1. Nederst på siden skal du vælge <img src='data:image/png;base64,' />
 **Personer**.
2. På værktøjslinjen skal du vælge **Administrer**  >  **import kontakter**.
3. Vælg **Gennemse**, Vælg din CSV-fil, og vælg derefter **Åbn**.
4. Vælg **Importér**.

Hvis du vil have mere at vide, skal du se [importere kontakter til Outlook](https://support.office.com/article/bb796340-b58a-46c1-90c7-b549b8f3c5f8#ID0EAACAAA=Outlook_on_the_web).

