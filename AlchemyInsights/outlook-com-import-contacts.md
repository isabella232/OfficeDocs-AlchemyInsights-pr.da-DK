---
title: Importere kontakter i Outlook.com
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.collection: Adm_O365
ms.custom:
- "278"
- "8000019"
ms.assetid: 921c5c7c-9a02-4682-9bd1-03ef9bb0fd6e
ms.openlocfilehash: 3ba007f265e580196e23cb636cab029784e255d8
ms.sourcegitcommit: c6692ce0fa1358ec3529e59ca0ecdfdea4cdc759
ms.translationtype: MT
ms.contentlocale: da-DK
ms.lasthandoff: 09/14/2020
ms.locfileid: "47723997"
---
# <a name="import-contacts"></a>Importere kontakter

Før du importerer kontakter til Outlook.com, skal du eksportere dem fra din anden mail udbyder som en. csv-fil. Derefter skal du følge vejledningen nedenfor.
  
1. Gå til [personer](https://outlook.live.com/people/) <img src='data:image/png;base64,' />
.
2. Øverst på siden skal du vælge **Administrer** \> **import kontakter**.
3. Vælg **Gennemse**, gå til den mappe, hvor du gemte de eksporterede kontakter, og vælg **Åbn**.

Hvis du vil have mere at vide, skal du se [importere kontakter til Outlook.com](https://support.office.com/article/285a3b55-8d93-4ac8-93df-43fffd13b2f1?wt.mc_id=Office_Outlook_com_Alchemy).